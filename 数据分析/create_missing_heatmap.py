...
def get_missing_data(db_path):
    """
    从错误统计数据库中读取missing数据
    ...
    for table_name in tables:
        # 从表名提取字节数
        original_table = table_name.replace('_missing_errors', '')
...
def create_missing_heatmap(db_path, output_path=None):
    """
    创建缺失错误热力图
    ...
    """
    print("正在读取缺失数据...")
    byte_counts, heatmap_data = get_missing_data(db_path)
...
def main():
    ...
    if len(sys.argv) < 2:
        print("使用方法:")
        print("  python create_missing_heatmap.py <error_stats数据库路径> [输出图片路径]")
        print("\n示例:")
        print("  python create_missing_heatmap.py 数据分析/分析结果/error_stats_gemini_2_5_pro.db")
        print("  python create_missing_heatmap.py 数据分析/分析结果/error_stats_gemini_2_5_pro.db missing_heatmap.png")
        return
    ...
    if len(sys.argv) > 2:
        ...
        output_path = os.path.join(db_dir, f"{db_name}_missing_heatmap.png")
    
    create_missing_heatmap(db_path, output_path)
